...
<div class="main-container">
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
            ...
            $modules = new Modules();
            // Récupération de toutes les filières avec leurs modules
            $allFilieresWithModules = $modules->getAllFilieresWithModules($_SESSION['user']->id_departement);

            // Organisation des modules par filière
            $filieres = [];
            foreach ($allFilieresWithModules as $record) {
                $filieres[$record->filiere_id]['nom_filiere'] = $record->nom_filiere;
                $filieres[$record->filiere_id]['modules'][] = $record;
            }
            ?>
                ...
                <?php foreach ($filieres as $filiere): ?>
                    <div class="pd-20 card-box mb-30">
                        <div class="table-responsive">
                            <div class="pd-20 card-box mb-30 text-center card-filiere">
                                <div class="card-header">
                                    <h5 class="card-title"><?php echo $filiere['nom_filiere']; ?></h5>
                                </div>
                                <div class="card-body">
                                    <?php if (!empty($filiere['modules']) && $filiere['modules'][0]->module_id !== NULL): ?>
                                        <table class="table table-striped table-hover">
                                            ...
                                            <tbody>
                                            <?php foreach ($filiere['modules'] as $index => $module): ?>
                                                <tr>
                                                    <th scope="row"><?= $index + 1; ?></th>
                                                    <td><?php echo $module->nom_module; ?></td>
                                                    <td><?php echo $module->nom_prof; ?></td>
                                                    <td><?php echo $module->nom_semestre; ?></td>
                                                    <td><a href="supprimer_module?id=<?= $module->module_id ?>" class="btn btn-danger"><span>Supprimer </span><i class="icon-copy dw dw-delete-3"></i></a></td>
                                                </tr>
                                            <?php endforeach; ?>
                                            </tbody>
                                        </table>
