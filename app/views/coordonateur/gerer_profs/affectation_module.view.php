        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                <div class="page-header">
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="title">
                                <h4>Affecter un module à un professeur</h4>
                            </div>
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix">
                        <div class="pull-left">
                            <h4 class="text-blue h4">Affecter un module à un professeur</h4>
                            <p class="mb-30">Sélectionnez le module et le professeur pour l'affectation :</p>
                        </div>
                    </div>
                    <?php
                    ...
                    } elseif (isset($_POST['id_semestre'])) {
                        $id_semestre = $_POST['id_semestre'];
                    } else {
                        // Définissez une valeur par défaut ou gérez l'erreur
                        $id_semestre = 1; // Valeur par défaut, à ajuster selon vos besoins
                        // Vous pouvez également afficher un message d'erreur ou rediriger l'utilisateur
                    }
                    $id_filiere = $_SESSION['user']->id_filiere;


                    // Instanciation de la classe Profs pour interagir avec les données des professeurs
                    $profs = new Profs();
                    $modules = new Modules();

                    // Récupération de tous les professeurs avec leur département basé sur l'id de la filière de l'utilisateur connecté
                    $professeurs = $profs->getProfsWithFiliere($id_filiere);

                    // Récupération de tous les modules pour la filière et le semestre spécifié
                    $allModules = $modules->getAllModlesWithFiliere($id_filiere, $id_semestre);

                    // Affichage des messages de succès s'il y en a, et les supprimer de la session
                    ...
                    <?php
                    // Affichage des messages d'erreur s'il y en a, et les supprimer de la session
                    ...
                    <!-- Formulaire pour affecter un module à un professeur -->
                    <form method="POST" action="affectation_module_coordonateur/store">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Sélectionner un module</label>
                                    <select class="custom-select2 form-control" name="module_id" style="width: 100%; height: 38px">
                                        <!-- Boucle pour lister les modules disponibles -->
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="custom-select2 form-control" name="professeur_id" style="width: 100%; height: 38px">
                                        <!-- Boucle pour lister les professeurs disponibles -->
                                        <?php foreach ($professeurs as $professeur): ?>
                                            <option value="<?= $professeur->professeur_id ?>"><?= $professeur->nom_prof ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                        </div>
                        <button type="submit" class="btn btn-primary">Affecter Module</button>
                    </form>
