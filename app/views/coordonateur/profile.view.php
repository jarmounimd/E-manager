...
<body>
    ...
    <div class="main-container">
					...
					<div class="row">
						<div class="col-xl-5 col-lg-5 col-md-5 col-sm-12 mb-30">
							<div class="pd-20 card-box ">
								...
								<img
										src="<?= !empty($data[0]->profile_pic) ? ASSETS . 'uploads/' . htmlspecialchars($data[0]->profile_pic) : ASSETS . 'vendors/images/user_coordonateur.svg' ?>"
										alt=""
										class="avatar-photo"
									/>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-body pd-5">
                                                    <div class="img-container">
                                                        <img id="image" src="<?= isset($data[0]->profile_pic) ? ASSETS . 'uploads/' . $data[0]->profile_pic : ASSETS . 'vendors/images/user_coordonateur.svg' ?>" alt="Picture" />
                                                    </div>
                                                    <!-- File input for choosing a new profile picture within the modal -->
                                                    <form action="update_profile_coor/updatePicture" method="POST" enctype="multipart/form-data">
								...
								<h5 class="text-center h5 mb-0"><?php echo  $data[0]->prenom_coord.' '. $data[0]->nom_coord; ?></h5>
        ...
        <div class="pd-20">
            ...
            <?php
            $tacheModel = new TaskChef();
            $tasks = $tacheModel->getTasksForCoor($_SESSION['user']->id);
            $currentDate = date('Y-m-d');

            if (!empty($tasks)): // Check if tasks exist
                $openTasks = [];
                $closedTasks = [];
            ...
            <div class="task-title row align-items-center">
                <div class="col-md-8 col-sm-12">
                    <h6>Tâches en Attente (<?= $openTasksCount ?> tâches)</h6>
                </div>
            </div>
            ...
            <div class="profile-task-list pb-30">
                <ul>
                    <?php foreach ($openTasks as $task): ?>
                        <li>
                            <div class="task-type"><?= htmlspecialchars($task->task_title) ?></div>
                            <?= htmlspecialchars($task->task_message) ?>
                            <div class="task-assign">
                                <div class="due-date">
                                    Date limite <span><?= htmlspecialchars(date('d F Y', strtotime($task->due_date))) ?></span>
                                </div>
                            </div>
                        </li>
                    <?php endforeach; ?>
                    ...
                    <?php foreach ($closedTasks as $task): ?>
                        <li>
                            <div class="custom-control custom-checkbox mb-5">
                                <input type="checkbox" class="custom-control-input" id="task-close-<?= $task->id ?>" checked="" disabled="" />
                                <label class="custom-control-label" for="task-close-<?= $task->id ?>"></label>
                            </div>
                            <div class="task-type"><?= htmlspecialchars($task->task_title) ?></div>
                            <?= htmlspecialchars($task->task_message) ?>
                            <div class="task-assign">
...
<div class="tab-pane fade" id="tasks" role="tabpanel">
    ...
    <!-- Tâches assignées Début -->
    <div class="task-title row align-items-center">
        <h5 class="col-md-10 col-sm-12">Assignez des tâches aux professeurs:</h5>
        <div class="col-md-8 col-sm-12">
            <h6>Tâches assignées (<?= $openTasksCount ?> tâches)</h6>
        </div>
        <div class="col-md-4 col-sm-12 text-right">
            <a href="task-add" data-toggle="modal" data-target="#task-add" class="bg-light-blue btn text-blue weight-500">
                <i class="ion-plus-round"></i> Ajouter
            </a>
        </div>
    </div>
            ...
            <ul>
                <?php foreach ($openTasks as $task): ?>
                    <li>
                        ...
                        <?= htmlspecialchars($task->task_message) ?>
                        <div class="task-assign">
                            Assigné à <?= htmlspecialchars($task->professor_name) ?>
                            <div class="due-date">
                                Date limite <span><?= htmlspecialchars(date('d F Y', strtotime($task->due_date))) ?></span>
                            </div>
                        </div>
    ...
    <?php else: ?>
        <p>Aucune tâche en Attente disponible pour le moment.</p>
    <?php endif; ?>
    <!-- Tâches assignées Fin -->
    ...
    <?php else: ?>
        <p>Aucune tâche clôturée disponible pour le moment.</p>
    <?php endif; ?>
    <!-- Tâches Clôturées Fin -->
					                    ...
					                    <ul>
						                <li>
						                <form action="tache_prof/index" method="POST">
								        <div class="form-group row">
									    <label class="col-md-4">Titre de tâche</label>
									    <div class="col-md-8">
										<input type="text" class="form-control" name="task_title" required />
									    </div>
								        </div>
								        <div class="form-group row">
									    <label class="col-md-4">Message de la tâche</label>
									    <div class="col-md-8">
										<textarea class="form-control" name="task_message" required></textarea>
									    </div>
								        </div>
								        <div class="form-group row">
									    <label class="col-md-4">Assigné à</label>
									    <div class="col-md-8">
										<?php
											$id_filiere = $_SESSION['user']->id_filiere;
											$profs = new Profs();
											$professeurs = $profs->getProfsWithFiliere($id_filiere);
										?>
										<select
										...
										>
											<?php foreach ($professeurs as $professeur): ?>
												<option value="<?= $professeur->professeur_id ?>">
													<?php echo 'Mr '.$professeur->nom_prof ?>
												</option>
											<?php endforeach; ?>
										</select>
									    </div>
									    ...
									    <label class="col-md-4">La date limite</label>
									    <div class="col-md-8">
	    ...
	    <div class="profile-setting">
             <form action="update_profile_coor/updatePersonalInfo" method="POST">
            ...
            <li class="weight-500 col-md-11">
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        type="text"
                        name="nom"
                        value="<?= htmlspecialchars($data[0]->nom_coord) ?>"
                        required
                    />
                </div>
                <div class="form-group">
                    ...
                    <input
                        ...
                        type="text"
                        name="prenom"
                        value="<?= htmlspecialchars($data[0]->prenom_coord) ?>"
                        required
                    />
                </div>
                <div class="form-group">
                    ...
                    <input
                        ...
                        type="email"
                        name="email"
                        value="<?= htmlspecialchars($data[0]->email_coord) ?>"
                        required
                    />
                </div>
                <div class="form-group">
